...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public virtual async Task<IPagedList<Product>> SearchProductsAsync_Phong(
            int pageIndex = 0, 
            int pageSize = int.MaxValue, 
            IList<int> categoryIds = null,
            IList<int> manufacturerIds = null,
            ...
            bool showHidden = false,
            bool? overridePublished = null,
            string searchByYearRelease = null)
        {
            //some databases don't support int.MaxValue
            if (pageSize == int.MaxValue)
                pageSize = int.MaxValue - 1;
            ...
            if (!string.IsNullOrEmpty(searchByYearRelease) && !searchByYearRelease.Equals("0"))
            {
                productsQuery = from p in productsQuery
                                  where p.ReleaseDates.Year == Convert.ToUInt32(searchByYearRelease)
                                  select p;
            }
